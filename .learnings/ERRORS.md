...
### Summary
The API key audit runner exists but is not executable, so direct invocation fails.
...
### Context
- Command attempted: `/home/davesalter/.openclaw/workspace/state/api-key-audits/run-api-key-audit.py`
- Task: daily API key health and spend audit cron run.
- Fallback command succeeded: `python3 /home/davesalter/.openclaw/workspace/state/api-key-audits/run-api-key-audit.py`
- The corrected run used the scheduled timestamp environment values and wrote the expected report and snapshot.

### Suggested Fix
Either run the audit script through `python3` in cron payloads, or set the executable bit on the runner if direct invocation is preferred.
...
### Summary
The API key audit runner has a shebang but is not executable, so direct invocation fails.
...
### Context
- Command attempted: direct execution of `state/api-key-audits/run-api-key-audit.py`.
- Task: daily API key health and spend audit cron.
- Fallback: run the script through `python3` with the same timestamp environment.

### Suggested Fix
Either call the audit runner as `python3 state/api-key-audits/run-api-key-audit.py` in cron payloads or set the executable bit deliberately.
...
---
## [ERR-20260813-001] api_key_audit_script_not_executable

**Logged**: 2026-08-13T08:00:00-07:00
**Priority**: low
**Status**: pending
**Area**: infra

### Summary
The daily API key audit script has a Python shebang but is not executable, so direct invocation failed.
...
- Fallback command succeeded: `python3 scripts/api-key-health-audit.py`.
- No credentials were printed or modified.

### Suggested Fix
Either run this audit script with `python3` in automation or mark it executable if direct invocation is intended.
